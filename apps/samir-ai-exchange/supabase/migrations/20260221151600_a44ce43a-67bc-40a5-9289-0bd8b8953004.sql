...
-- 1. Balance Transfers Audit Table
CREATE TABLE public.balance_transfers_audit (
...
ALTER TABLE public.balance_transfers_audit ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins can manage transfer audit"
ON public.balance_transfers_audit FOR ALL
USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can view transfer audit"
ON public.balance_transfers_audit FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

-- 2. Atomic Transfer RPC with row locking
CREATE OR REPLACE FUNCTION public.execute_safe_transfer(
  p_agent_id uuid,
  p_amount numeric,
  p_direction text, -- 'to_agent' or 'to_master'
  p_admin_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
